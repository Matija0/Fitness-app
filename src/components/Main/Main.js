...
const Main = () => {
  ...
  const dbCollectionRef = collection(db, `${weekday[d.getDay()].toLowerCase()}`);
  ...
  return (
    <div id="container" className=" w-full   flex flex-row gap-5">
      <div
        style={{ minHeight: "500px" }}
        className="bg-gray-800 border border-gray-500 w-3/5 rounded-lg py-2"
      >
        <h1 className="text-gray-200 text-2xl font-semibold text-center">
          {currentDay}
        </h1>
        {data.length==0? (<h2 className=" text-2xl text-gray-200 font-light text-center mt-20">Rest Day!</h2>) : 
        (<div className="mt-7">
      ...
      <div className=" w-2/5 bg-gray-800 border border-gray-500 rounded-lg"></div>
    </div>
  );
};
